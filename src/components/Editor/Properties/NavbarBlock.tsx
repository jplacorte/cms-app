...
export default function NavbarBlock({data, weightOptions, handleChange, }: Props) {
  return (
    <div className="space-y-4">
                ...
                <Row>
                  <NumberInput label="Font Size (px)" value={data.linkFontSize || 14} onChange={(v) => handleChange("linkFontSize", v)} min={10} max={32} />
                  <SelectInput label="Font Weight" value={data.linkFontWeight || "500"} onChange={(v) => handleChange("linkFontWeight", v)} options={weightOptions} />
                </Row>
                <Row>
                  <ColorInput label="Link Color" value={data.linkColor || "#475569"} onChange={(v) => handleChange("linkColor", v)} />
                  <ColorInput label="Hover Color" value={data.linkColorHover || "#2563eb"} onChange={(v) => handleChange("linkColorHover", v)} />
                </Row>
                <Row>
                  <ColorInput label="Dropdown BG" value={data.dropdownBgColor || "#ffffff"} onChange={(v) => handleChange("dropdownBgColor", v)} />
                </Row>
    
                <div className="pt-2">
                  {(data.links as NavLink[] || []).map((link: NavLink, index: number) => (
                    <div key={index} className="space-y-2 border border-slate-200 p-3 rounded-lg bg-slate-50 mb-3">
                      <div className="flex gap-2 items-center">
                        <input type="text" value={link.label} onChange={(e) => {
                          const l = [...(data.links as NavLink[] || [])]; 
                          l[index] = { ...l[index], label: e.target.value }; 
                          handleChange("links", l);
                        }} className="flex-[0.8] min-w-0 text-sm text-black border border-slate-300 rounded p-2" placeholder="Label" />
                        <input type="text" value={link.href} onChange={(e) => {
                          const l = [...(data.links as NavLink[] || [])]; 
                          l[index] = { ...l[index], href: e.target.value }; 
                          handleChange("links", l);
                        }} className="flex-1 min-w-0 text-sm text-black border border-slate-300 rounded p-2" placeholder="/path" />
                        <button onClick={() => {
                          const l = [...(data.links as NavLink[] || [])]; 
                          l.splice(index, 1); 
                          handleChange("links", l);
                        ...
                        {(link.sublinks || []).map((sublink, subIndex) => (
                          <div key={subIndex} className="flex gap-2 items-center">
                            <input type="text" value={sublink.label} onChange={(e) => {
                              ...
                              handleChange("links", l);
                            }} className="flex-[0.8] min-w-0 text-xs text-black border border-slate-200 rounded p-1.5" placeholder="Sub-label" />
                            <input type="text" value={sublink.href} onChange={(e) => {
                              ...
                              handleChange("links", l);
                            }} className="flex-1 min-w-0 text-xs text-black border border-slate-200 rounded p-1.5" placeholder="/sub-path" />
                            <button onClick={() => {
                              const l = [...(data.links as NavLink[] || [])]; 
                              l[index] = { ...l[index] }; 
                              l[index].sublinks = [...(l[index].sublinks || [])];
                ...
                <Row>
                  <SelectInput label="Max Width" value={data.maxWidth || "1200px"} onChange={(v) => handleChange("maxWidth", v)} options={[
                ...
                <Row>
                  <NumberInput label="Padding X" value={data.paddingX ?? 32} onChange={(v) => handleChange("paddingX", v)} />
                  <NumberInput label="Padding Y" value={data.paddingY ?? 0} onChange={(v) => handleChange("paddingY", v)} />
                </Row>
                <Row>
                ...
                <TextInput label="Button Text" value={data.cta2Text || ""} onChange={(v) => handleChange("cta2Text", v)} placeholder="e.g. Login" />
                <TextInput label="Button Link" value={data.cta2Link || ""} onChange={(v) => handleChange("cta2Link", v)} />
                
                <Row>
                  <NumberInput label="Font Size (px)" value={data.cta2FontSize ?? 14} onChange={(v) => handleChange("cta2FontSize", v)} />
                ...
                <Row>
                  ...
                  <ColorInput label="Text Color" value={data.cta2TextColor || "#0f172a"} onChange={(v) => handleChange("cta2TextColor", v)} />
                </Row>
                <Row>
                  <ColorInput label="Hover BG" value={data.cta2HoverBgColor || "#f8fafc"} onChange={(v) => handleChange("cta2HoverBgColor", v)} />
                  <ColorInput label="Hover Text" value={data.cta2HoverTextColor || "#0f172a"} onChange={(v) => handleChange("cta2HoverTextColor", v)} />
                </Row>
                <Row>
                  <NumberInput label="Padding X" value={data.cta2PaddingX ?? 20} onChange={(v) => handleChange("cta2PaddingX", v)} />
                  <NumberInput label="Padding Y" value={data.cta2PaddingY ?? 8} onChange={(v) => handleChange("cta2PaddingY", v)} />
                </Row>
                <Row>
                ...
                <ColorInput label="Border Color" value={data.cta2BorderColor || "#e2e8f0"} onChange={(v) => handleChange("cta2BorderColor", v)} />
    
                <SectionHeading>Primary CTA Button</SectionHeading>
                <TextInput label="Button Text" value={data.ctaText || ""} onChange={(v) => handleChange("ctaText", v)} placeholder="e.g. Sign Up" />
                <TextInput label="Button Link" value={data.ctaLink || ""} onChange={(v) => handleChange("ctaLink", v)} />
                
                <Row>
                  <NumberInput label="Font Size (px)" value={data.ctaFontSize ?? 14} onChange={(v) => handleChange("ctaFontSize", v)} />
                  <SelectInput label="Font Weight" value={data.ctaFontWeight || "600"} onChange={(v) => handleChange("ctaFontWeight", v)} options={weightOptions} />
                </Row>
    
                <Row>
                  <ColorInput label="Background" value={data.ctaBgColor || "#2563eb"} onChange={(v) => handleChange("ctaBgColor", v)} />
                  <ColorInput label="Text Color" value={data.ctaTextColor || "#ffffff"} onChange={(v) => handleChange("ctaTextColor", v)} />
                </Row>
                <Row>
                  <ColorInput label="Hover BG" value={data.ctaHoverBgColor || "#1d4ed8"} onChange={(v) => handleChange("ctaHoverBgColor", v)} />
                  <ColorInput label="Hover Text" value={data.ctaHoverTextColor || "#ffffff"} onChange={(v) => handleChange("ctaHoverTextColor", v)} />
                </Row>
                <Row>
                  <NumberInput label="Padding X" value={data.ctaPaddingX ?? 20} onChange={(v) => handleChange("ctaPaddingX", v)} />
                  <NumberInput label="Padding Y" value={data.ctaPaddingY ?? 8} onChange={(v) => handleChange("ctaPaddingY", v)} />
                </Row>
                <Row>
